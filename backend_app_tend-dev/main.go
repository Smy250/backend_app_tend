...
func main() {
	...
	if apiKey == "" {
		log.Fatalf("Error fatal: GEMINI_API_KEY no encontrada en las variables de entorno. Por favor, configúrela en su archivo .env")
	}
	//Para inicializar el cliente de la IA de forma global
	if err := apis.InitGeminiClient(context.Background(), apiKey); err != nil {
		log.Fatalf("Error al inicializar el cliente de IA: %v", err)
	}
...
/*
	...
	* 9. Analisis de archivo .pdf con la IA
	Estoy verificando si se clonó correctamente*/
