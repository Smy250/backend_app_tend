...
// Nuevo controlador, solo se agregó la funcionalidad para leer el archivo .pdf
// SummarizePDFRequest define el formato del JSON de entrada para este endpoint.
type SummarizePDFRequest struct {
	FilePath string `json:"file_path" binding:"required"` // Ruta al archivo PDF en el servidor
	Prompt   string `json:"prompt"`                       // Prompt opcional para la IA
	ModelID  uint64 `json:"model_id"`                     // Índice del modelo a usar
}

// POST_SummarizePDF es el controlador que maneja la solicitud de resumen de PDF.
func POST_SummarizePDF(ctx_gin *gin.Context) {
	var req SummarizePDFRequest
	if err := ctx_gin.ShouldBindJSON(&req); err != nil {
		ctx_gin.JSON(http.StatusBadRequest, gin.H{"error": fmt.Sprintf("Formato de JSON inválido o campos faltantes: %v", err)})
		return
	}

	// Llama a la nueva función del paquete apis para el resumen
	summary, err := apis.SummarizePDFAPI(req.FilePath, req.Prompt, req.ModelID)
	if err != nil {
		ctx_gin.JSON(http.StatusInternalServerError, gin.H{"error": fmt.Sprintf("Error al procesar el resumen del PDF: %v", err.Error())})
		return
	}

	ctx_gin.JSON(http.StatusOK, gin.H{"summary": summary})
}
